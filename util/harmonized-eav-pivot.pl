...
while($line = <FH>) {
		...
		$id = $data[0]; 	# collect id/accession number

		if ($firstIter > 0) {
				$prevId = $id; # on first iteration set the previous id
		...
		if ($prevId ne $id) {
				...
				print "$prevId"; # print id/accession number; note this will be the previous id
				...
				$prevId = $id; # update the previous id
		}
...
print "$id"; # print id/accession number
foreach $value (values %attributes) { print "\t$value";} 	# print attribute values
print "\n"; # end of line
