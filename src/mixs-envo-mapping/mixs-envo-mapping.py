...
def get_package_dictionary():
    ...
    bios_columns = ['Name', 'display_name', 'short_name', 'env_package', 'env_package_display', 'not_appropriate_for',
                    'description', 'Example']
    bios_df = pds.DataFrame(columns=bios_columns)

    bios_root = bios_doc.getroot()
    for node in bios_root:
        package_name = node.find('Name').text if node is not None else None
        display_name = node.find('display_name').text if node is not None else None
        short_name = node.find('short_name').text if node is not None else None
        description = node.find('description').text if node is not None else None
        env_package = node.find('env_package').text if node is not None else None
        env_package_display = node.find('env_package_display').text if node is not None else None
        not_appropriate_for = node.find('not_appropriate_for').text if node is not None else None
        package_example = node.find('Example').text if node is not None else None
        bios_df = bios_df.append(pds.Series([package_name,
                                             display_name,
                                             short_name,
                                             env_package,
                                             env_package_display,
                                             not_appropriate_for,
                                             description,
                                             package_example], index=bios_columns), ignore_index=True)
