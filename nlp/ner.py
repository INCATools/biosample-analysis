...
INPUT_DIRECTORY = '../downloads/'#'data/input/'
...
df_ncbi = pd.read_csv(os.path.join(INPUT_DIRECTORY,'nmdc-biotable-for-ner-20201016.tsv'), low_memory=False, sep='\t')


# Column conctenation for NLP
#df_ncbi['studyBiosampleId'] = df_ncbi['StudyId']+'_'+df_ncbi['BiosampleId']
df_ncbi['id'] = df_ncbi['GOLD_ID']
df_ncbi['text'] = df_ncbi['BIOSAMPLE_NAME'] + '. ' + df_ncbi['DESCRIPTION'] + '. ' + df_ncbi['HABITAT'] +'. '+df_ncbi['SAMPLE_COLLECTION_SITE']

#df_gold['text'] = df_gold['name'] + '. ' + df_gold['name'] + '. ' + df_gold['community'] + '. ' + df_gold['habitat']

nlp_ncbi_df = df_ncbi[['id', 'text']]
